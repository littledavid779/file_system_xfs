...
#include "/usr/src/linux-6.8/include/asm-generic/access_ok.h"

// #define printk if(0)_printk
// #define SUBMIT_TEST
// #ifdef SUBMIT_TEST
// static void xfs_test_bio_end_io(struct bio *bio)
// static int xfs_submit_test(void)
// #endif
...
// #endif
	...
	// printk("[BIO MERGE SINGLE PAGE BUF INFO]: ");
	// for(int i = 0;i < len;i++){
	// 	printk(KERN_CONT "%c", *((char *)(allpage_data) + 1ll * pageid * Aligned_Size + page_in_pos + i));
	// }
	// if(copyret != 0){
...
int dio_page_num = 3;
int using_page_num_threshold = 4096 * 0.6;
void mydelay(void){
...
void calc_LR_size(size_t user_pos, size_t len, size_t *L_Size, size_t *R_Size){
	if((user_pos/Aligned_Size) == ((user_pos + len - 1)/Aligned_Size)) //请求在一个blade page中
	{	
		// printk("trace flag-1*************start!!!");
		size_t result1 = user_pos/Aligned_Size;
		// TODO: 这里的计算有问题，应该是 (user_pos + from->count - 1)/Aligned_Size
		size_t result2 = (user_pos + len - 1)/Aligned_Size;
	...
	else
	{
		*L_Size = Aligned_Size - user_pos%Aligned_Size;
		if(*L_Size == Aligned_Size)
			*L_Size = 0;
		*R_Size = (user_pos + len)%Aligned_Size;
		// printk("L_Size = %ld, R_Size = %ld, user_pos Aligned_Size = %ld\n",L_Size,R_Size,user_pos%Aligned_Size);
	}
	if(( (user_pos/Aligned_Size) == ((user_pos + len - 1)/Aligned_Size) ) && len == Aligned_Size){
		*L_Size = *R_Size = 0;
	}
}
EXPORT_SYMBOL_GPL(calc_LR_size);
	...
	struct iov_iter		*from)
{
	...
	ssize_t initial_from_count = from->count;
...
// #endif
	...
	if (iocb->ki_flags & IOCB_DIRECT) 
	{
		...
		if (tmp != -ENOTBLK){
			iocb->ki_pos = initial_ki_pos + initial_from_count;
			from->count = 0;
			return RET;
		}
		// if (ret != -ENOTBLK)
...
io_done:
	...
	// printk("io done return ret = %ld",RET);
	iocb->ki_pos = initial_ki_pos + initial_from_count;
	from->count = 0;
	return RET;
	...
	struct iov_iter		*from)
{
	...
	if (ocount == 0)
		...
		/* 测试用的 bio end_io 回调 */
	void xfs_test_bio_end_io(struct bio *bio)
	{
		complete(bio->bi_private);
	}

	/* 测试提交 bio 的函数 */
	int xfs_submit_test(void)
	{
		...
		char *data_buf = NULL;
		int ret;
		sector_t sector = 0; /* 测试写入的起始扇区 */



		/* 打开指定块设备  */
		bdev_handle  = bdev_open_by_path("/dev/nvme3n1", FMODE_WRITE | FMODE_EXEC, current_user_ns(), NULL);
		if (IS_ERR(bdev_handle)) {
			pr_err("bdev_open_by_path failed: %ld\n", PTR_ERR(bdev_handle));
			return PTR_ERR(bdev_handle);
		}
		// bdev = bdev_handle_get(bdev_handle);
		...
		/* 用测试数据填充整页 */
		memset(data_buf, 0xAA, PAGE_SIZE);
		...
		bio->bi_end_io = xfs_test_bio_end_io;
		bio->bi_private = &done;

		/* 将整页数据添加到 bio，起始偏移为 0 */
		if (bio_add_page(bio, virt_to_page(data_buf), PAGE_SIZE, offset_in_page(data_buf)) != PAGE_SIZE) {
		...
		if (!wait_for_completion_io_timeout(&done, msecs_to_jiffies(5000))) {
			pr_err("Bio completion timeout\n");
			ret = -ETIMEDOUT;
			goto out_bio_put;
		}
		if (bio->bi_status != BLK_STS_OK) {
			pr_err("bio write failed with status %d\n", bio->bi_status);
			ret = -EIO;
		} else {
			pr_info("bio write succeeded\n");
			ret = 0;
		}
	out_bio_put:
		bio_put(bio);
	...
	if (iocb->ki_flags & IOCB_DIRECT) {
		 ...
		 */
		ret = xfs_file_dio_write(iocb, from);
		// printk("dio iocb_pos = %ld, from_count = %ld", iocb->ki_pos, from->count);
		if (ret != -ENOTBLK)
			return ret;
	}

	// ssize_t rtn = xfs_file_buffered_write(iocb, from);
	// printk("bio iocb_pos = %ld, from_count = %ld", iocb->ki_pos, from->count);
	// return rtn;
	...
	struct iov_iter		*to)
{
	...
	// if(initial_to_count == 4128 && initial_ki_pos == 30404577){
	// 	printk("[READ ITER]: iocb->ki_pos = %ld len = %ld inode = %ld filelen = %ld", iocb->ki_pos, to->count, XFS_I(inode)->i_ino, inode->i_size);
	// }
	// printk("[READ ITER READ BUF]: ");
	// void *tempbuf0 = vmalloc(to->count);
	// for(int i = 0;i < to->count;i++){
	// 	((char *)tempbuf0)[i] = 0;
	// }
	// copy_to_user(to->ubuf,tempbuf0,to->count);
	// copy_from_user(tempbuf0,to->ubuf,to->count);
	// for(int i = 0;i < to->count;i++){
	// 	printk(KERN_CONT "%d ",((char *)tempbuf0)[i]);
	// }
	// vfree(tempbuf0);
	// tempbuf0 = NULL;
	...
	// // 请求段完全非法，直接返回
	// if(iocb->ki_pos > inode->i_size || inode->i_size == 0){
	// 	return RET;
	// }
	// // 请求段部分非法，截断请求段
	// if(iocb->ki_pos + to->count > inode->i_size){
	// 	to->count = inode->i_size - iocb->ki_pos;
	// }
...
// #define DEBUG
	...
	while(cur_offset < end_offset){
		...
		if(legal_io){
			...
			// buffered_read_ret = xfs_file_buffered_read(iocb, to);
	...
	// for(int i=0;i<1;i++)
	{
		...
		// else ret += copy_to_user((to->ubuf), (char *)(memorycache)+(Aligned_Size+Page_Meta_Size)*blade_io[i].pageid+Page_Meta_Size+blade_io[i].pos,blade_io[i].count); 
			...
			// printk("[COPY TO USER PRE BUF INFO]: ");
			// for(int j = 0;j< blade_io[i].count;j++){
			// 	printk(KERN_CONT "%c", *((char *)(allpage_data) + 1ll * blade_io[i].pageid * Aligned_Size + blade_io[i].pos + j));
			// }
...
io_done:
	...
	// return ret;
	// if(initial_to_count != RET){
	// 	printk("to_count = %ld, but RET = %ld. buf_read_ret = %ld blade_read_ret = %ld",initial_to_count, RET, buf_read_ret, blade_read_ret);
	// }
	iocb->ki_pos = initial_ki_pos + RET;
	to->count = 0;
	return RET;
...
bio_only:
	...
	// if(initial_to_count != ret){
	// 	printk("to_count = %ld, but ret = %ld",initial_to_count, ret);
	// }
	iocb->ki_pos = initial_ki_pos + ret;
	to->count = 0;
	return ret;
	...
	struct iov_iter		*to)
{
	...
	if (ret > 0)
		XFS_STATS_ADD(mp, xs_read_bytes, ret);
	// printk("iocb->ki_pos = %ld to->count = %ld ret = %ld\n",iocb->ki_pos,to->count,ret);
	// if(ret != initial_to_count){
	// 	printk("to_count = %ld,but ret = %ld",initial_to_count,ret);
	// }
	return ret;
}
