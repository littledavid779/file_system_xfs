	...
	struct iomap		*srcmap)
{
	...
	xfs_fileoff_t		offset_fsb = XFS_B_TO_FSBT(mp, offset);
	// 计算从字节偏移量转换为xfs的基本块偏移
	xfs_fileoff_t		end_fsb = xfs_iomap_end_fsb(mp, offset, length);
	// 计算操作结束时对应的块号，用于确定此次io操作所覆盖的文件范围
	...
	if (error)
		return error;
	// 以上为写入前检查
	// 查找给定文件范围内已经分配的extents的函数 查找现有extent
	//查
	...
	// 处理NOWAIT 和 OVERWRITE ONLY类型的IO操作
	/*
	 ...
	 */
	 // IOMAP_OVERWRITE_ONLY 表示该操作只能覆盖已经分配的extents 不能分配新的extents
...
allocate_blocks:
	...
	// bmbt_to_iomap() 将找到的或新分配的extent转换为iomap结构。
	...
	struct iomap		*srcmap)
{
	...
	/* if (imap.br_startoff <= offset_fsb) 
	用于判断当前的 extent 是否能够覆盖写入操作的起始位置。
	如果条件为 true，则表示当前 extent 可以处理这个写入操作的开始部分；
	如果为 false，则说明写入起点未被分配，可能需要进一步查找其他映射或分配新块。
	*/
	if (imap.br_startoff <= offset_fsb) {  // 写入操作的位置位于当前找到的extent范围内的情况
		/*
		...
		// 用于修剪当前的 extent，使其只包含写操作所需要的范围。也就是说，它将 extent 修剪到从 offset_fsb（写入起始位置）到 end_fsb（写入结束位置）的范围。
		...
		end_fsb = imap.br_startoff + imap.br_blockcount;
	} else { // 当写操作的位置不在现有的映射范围内时，进行合适的"分配准备"工作，特别是限制写入长度和处理 CoW 文件。
		/*
	...
	/*检测写操作是否超出了文件的当前大小，并且文件是否在文件末尾（EOF，End of File）处进行写入*/
	if (eof && offset + count > XFS_ISIZE(ip)) { // 超过了文件大小，扩展文件
		/*
		 * Determine the initial size of the preallocation.
		 * We clean up any extra preallocation when the file is closed.
		 */
		// 所有的iomao_prealloc_size()会计算出一个合适的预分配大小
		if (xfs_has_allocsize(mp))// 文件系统拥有分配大小限制，是否挂载了allocsize
			prealloc_blocks = mp->m_allocsize_blocks; // 分配大小，对应文件系统的分配块数，用于预分配
		else if (allocfork == XFS_DATA_FORK) // 是否正在处理数据块
			prealloc_blocks = xfs_iomap_prealloc_size(ip, allocfork,
						offset, count, &icur);// 计算预分配的块数
		else // 是否在处理cow块
			prealloc_blocks = xfs_iomap_prealloc_size(ip, allocfork,
						offset, count, &ccur); // 计算预分配的块数
		if (prealloc_blocks) { // 调整预分配的大小和对齐
			xfs_extlen_t	align;
