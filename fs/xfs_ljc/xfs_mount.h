...
// #define XFS_PRINTK // 是否启用 XFS 输出信息: open close read write fsync
#define XXFS // 是否启用 XXFS
#define DOFLUSH // 是否启用 FLUSH
// #define ONLY_FLUSH // 只启用 FLUSH, 开启 DOFLUSH 时有效
// #define ONLY_READSSD // 只启用 READSSD, 开启 DOFLUSH 时有效
#define USING_LOCK // 是否启用锁
// #define NEW_BUFFERED_READ_STRATEGY // 是否启用新的缓存读策略,注意 旧 的缓存读策略在 Aligned_Size ≥ 128KB 时会出现 double free 的问题
// #define NO_MAGNIFICATION_READ_STRATEGY // 是否启用不放大的读策略
#define BASIC_READ_STRATEGY // 是否启用基本的读策略,即一次全部读取
// #define DYNAMIC_PAGE_SIZE // 是否启用动态页大小(被废弃)
#define STATIC_WRITE_ITER_DIO_BUF // 是否启用静态 DIO 缓冲区
#define STATIC_RANGE_QUERY_RESULT // 是否启用静态范围查询结果
#define STATIC_BLADE_IO // 是否启用静态 blade io
// #define BOTH_ENDS_ALLOC // 是否启用两端分配 alloc page
// #define OPEN_USELESS_ALLOC // 是否启用无用的分配
#define FLEXIBLE_DIO_WRITE // 是否启用灵活的 DIO 写
// #define MYDEBUG // 是否开启调试输出
#define OPEN_PRINTK // 是否开启 printk 输出
// #define CALCULATE_PER4K // 计算 4K 满页所占的比例
// #define CALCULATE_STORAGE_PERCENT // 计算持久化比例
#define MAX_CALCULATE_RECORDS (200000ll) // 计算最大记录数



#ifdef XXFS

#define Size4K (4ll*1024)
#define Size16K (16ll*1024)
#define Size32K (32ll*1024)
#define Size64K (64ll*1024)
#define Size128K (128ll*1024)
#define Size256K (256ll*1024)
// void *memorycache=NULL;
#define MemoryCacheSize (60ll * 1024 * 1024 * 1024)
#define XARRAY_SIZE (10000000ll)
#define MAX_CIRCULAR_QUEUE (10000000) // 循环队列的最大长度
#define WriteIterDIOBufSize (6ll*1024*1024) // 10M
#define WriteIterDIOBufNum (16)
#define NO_MAGANIFICATION_READ_BUF_SIZE (10ll * 1024 * 1024)
#define Aligned_Size Size256K
#define Chunk_Size Size256K
#define Page_Meta_Size (1ll*1024)
// #define Memory_Size (10ll*1024*1024*1024)
#define FLUSH_THRESHOLD (256ll*1024)
//维护多个flushing线程，根据忙碌情况自适应下刷
#define USER_THREAD_NUM 1
#define FLUSH_THREAD_NUM 2
#define READ 0
#define WRITE 1

#define BUSY_THRESHOLD 16
typedef long long BLADE_PAGE_OFFSET;
typedef enum {
	IO_READ = 0,
	IO_WRITE = 1
} IO_TYPE;
...
extern ssize_t write_iter_opt;

#ifdef CALCULATE_PER4K
#define BLADE_PAGE_4K_NUM (MemoryCacheSize / Size4K)
struct blade_page_4k {
	int pos[127];
	int len[127];
	int num;
	int sumlen;
};
extern struct blade_page_4k *allpage_4k;
extern ssize_t used_page_4k_num;
extern ssize_t full_page_4k_num;
extern ssize_t *used_page_4k_num_array;
extern ssize_t *full_page_4k_num_array;
extern void bio_merge_single_page_4k(int pageid, int page_in_pos, int len);
#endif

#ifdef CALCULATE_STORAGE_PERCENT
extern ssize_t sum_storage_bytes;
extern ssize_t sum_storage_bytes_include_delete; 
extern ssize_t *sum_storage_bytes_array;
extern ssize_t *sum_storage_bytes_include_delete_array;
extern ssize_t *sum_write_bytes_array;
#endif


#ifdef DYNAMIC_PAGE_SIZE
// 每一段的大小，每一段中页的大小是相同的，不同段按照 256KB,64KB,16KB 的顺序来规定页的大小
// 暂定每一段的大小为 1GB
#define SEGMENT_SIZE (1ll*1024*1024*1024)
// PAGE_SIZE_TYPE 用于描述页的大小,取值可能是 Size16K,Size32K,Size64K,Size128K,Size256K
typedef enum {
	BLADE_PAGE_TYPE_CONTROL, // 控制页
	BLADE_PAGE_TYPE_DATA	 // 纯数据页
};
#endif
// 描述一个页,严格保证除了 data 之外的数据恰好是 1KB(1024B) 大小
struct blade_page_meta // 1KB
{
	// 127 * 4B = 508B
	unsigned int pos[127];
	// 127 * 4B = 508B
	unsigned int len[127];
	// 4B
	unsigned int num;
	// 4B
	unsigned int user_pageid; // file logical block id

	// char padding[3072];
};
struct blade_page_data // Align_Size 256KB
{
	// // 8B 描述当前页的大小
	// BLADE_PAGE_SIZE_TYPE page_size;
	// // 8B 描述当前页的类型
	// BLADE_PAGE_TYPE page_type;
	// 具体数据
	char data[Aligned_Size];
};
struct blade_page {
	struct blade_page_meta meta;
	struct blade_page_data data;
};
typedef struct {
	size_t pageid;
	size_t userpageid;
...
extern struct blade_page_meta *allpage_meta;
extern struct blade_page_data *allpage_data;
extern void *memorycache;
extern int *blade_page_bitmap;
extern int blade_page_pos;
extern unsigned short *myxa_val;
extern struct xarray **myxa;
extern atomic_t *ready_blade_page_num;
extern atomic_t *unready_blade_page_num;
extern atomic_t *SSD_task_num;
extern spinlock_t *flush_queue_lock;
extern spinlock_t *readssd_queue_lock;
extern spinlock_t *blade_page_lock;
extern spinlock_t *bitmap_lock;
extern atomic64_t page_used_size;
extern void** write_iter_dio_buf;
extern BOOL* write_iter_dio_buf_used;
extern spinlock_t write_iter_dio_buf_lock;
extern long long** static_range_query_result;
extern io_request** static_blade_io;
extern BOOL* static_range_query_result_used;
extern BOOL* static_blade_io_used;
extern spinlock_t static_range_query_result_lock;
extern spinlock_t static_blade_io_lock;
extern size_t max_blade_page_io_num;
extern size_t max_blade_page_num;
extern size_t max_dio_buf_len;
extern size_t max_range_query_result_num;
extern size_t max_read_blade_io_num;
extern size_t *xarray_hash_code_table;
extern struct xarray *file_inode_xa;
extern size_t dio_write_opt_num;
extern size_t dio_write_page_num;
extern size_t max_using_page_num;
extern size_t using_page_num;
extern spinlock_t *file_xarray_lock;
extern size_t sum_buffered_read_request_len;
extern size_t sum_buffered_read_success_len;

extern atomic64_t flush_write_blocked;
extern atomic64_t flush_read_blocked;

extern struct timespec64 last_real_io_time;
extern struct timespec64 now_time;
extern struct timespec64 mount_time;
extern struct timespec64 first_rw_time;
extern struct timespec64 flush_end_time;

extern size_t print_kernel_state_opt;
extern ssize_t sum_copy_from_user_bytes;
extern ssize_t sum_failed_copy_from_user_bytes;
extern ssize_t sum_write_bytes;
extern ssize_t sum_read_bytes;

extern void* no_maganification_read_buf;


#ifdef BOTH_ENDS_ALLOC
extern spinlock_t top_alloc_lock;
extern spinlock_t bottom_alloc_lock;
extern size_t top_blade_page_pos;
extern size_t bottom_blade_page_pos;
#endif

extern ssize_t *blg;
extern int blg_ed;
extern spinlock_t blg_lock;
...
typedef struct {
	Queue_Info queue[MAX_CIRCULAR_QUEUE]; // 存储队列的数组
	int front; // 队头
	int rear; // 队尾
} CircularQueue;
typedef struct {
...
extern FlushIOInfo *flush_io_info;
extern ReadSSDIOInfo *readssd_io_info;

extern CircularQueue **flush_circular_queue;
extern CircularQueue **readssd_circular_queue;
// 循环队列的方法声明
...
// 自己加的
extern ssize_t xfs_file_dio_write(struct kiocb *iocb, struct iov_iter *from);
extern ssize_t xfs_file_buffered_read(struct kiocb *iocb, struct iov_iter *to);
extern ssize_t xfs_file_dio_read(struct kiocb *iocb, struct iov_iter *to);

#ifdef XXFS

extern void show_time(void);
extern int deep_copy_kiocb(struct kiocb *src, struct kiocb **dst);
extern int deep_copy_iov_iter(const struct iov_iter *src, struct iov_iter **dst,BOOL is_user_copy);
extern void init_xarray(struct xarray **xa);
...
extern void set_now_time(struct timespec64 *now);
extern void reset_time(struct timespec64 *time);
extern struct timespec64 TIME_NSEC_TO_SEC(long long nsec);
extern long long TIME_MSEC_TO_NSEC(long long msec);
extern long long TIME_NSEC_TO_MSEC(long long nsec);
extern long long get_time_diff_sec(struct timespec64 *start, struct timespec64 *end);
extern long long get_time_diff_nsec(struct timespec64 *start, struct timespec64 *end);
extern BOOL is_zero_time(struct timespec64 *time);
extern void print_kernel_state(void);
extern size_t get_hashcode(long value);
extern inline ssize_t mymin(ssize_t a, ssize_t b);
extern inline ssize_t mymax(ssize_t a, ssize_t b);
