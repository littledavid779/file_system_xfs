	...
	bool			*allocated)
{
	...
	if (*len || args->alignment > 1) {
		...
		do {
			error = xfs_alloc_get_rec(acur->cnt, bno, len, &i);// 从当前 B+ 树光标 acur->cnt 中获取一个空闲块记录
			if (error)
				return error;
			if (XFS_IS_CORRUPT(args->mp, i != 1))
				return -EFSCORRUPTED;
...
restart:
	...
	if (xfs_btree_islastblock(acur.cnt, 0)) {
		...
		// 最后一个叶节点策略：如果光标指向的 B+ 树块是最后一个叶节点，则尝试分配树中较大的空闲块。
	 ...
	 * locality.
	 */
	// xfs_alloc_ag_vextent_locality 算法尝试根据块号和空闲空间 B+ 树，找到与目标块号靠近的空闲块，以优化分配局部性
	...
	xfs_fsblock_t		target)
{
	...
	error = xfs_alloc_vextent_iterate_ags(args, minimum_agno, start_agno,
			XFS_FSB_TO_AGBNO(mp, target), alloc_flags);// 迭代，在ag中找到符合条件的块
	...
	xfs_fsblock_t		target)
{
	...
	error = xfs_alloc_vextent_check_args(args, target, &minimum_agno); // 检查传递参数
	if (error) {
		if (error == -ENOSPC)
			return 0;
		return error;
