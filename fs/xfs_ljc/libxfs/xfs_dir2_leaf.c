 ...
 */
int
xfs_dir2_leaf_lookup( // 在叶节点中查找目录项(指定的文件名) 并返回对应的inode编号
	xfs_da_args_t		*args)		/* operation arguments */
{
	struct xfs_buf		*dbp;		/* data block buffer */
	...
	xfs_dir2_leaf_entry_t	*lep;		/* leaf entry B+树叶节点中的目录项索引*/
	xfs_trans_t		*tp;		/* transaction pointer */
	struct xfs_dir3_icleaf_hdr leafhdr;
	 ...
	 * Look up name in the leaf block, returning both buffers and index.
	   根据文件名的hash值在B+树的叶节点中查找文件名，并返回对应的位置。
	 */
	error = xfs_dir2_leaf_lookup_int(args, &lbp, &index, &dbp, &leafhdr);
	/*查找叶节点的核心部分。该函数会在 B+树的叶节点中查找目录项，
	  并返回匹配的索引位置（index）
	  、叶节点缓冲区（lbp）、数据块缓冲区（dbp），以及叶节点头信息（leafhdr）。
	*/
	if (error)
		return error;
	...
	tp = args->trans;
	dp = args->dp;
	xfs_dir3_leaf_check(dp, lbp); // 检查叶节点的合法性，确保没有损坏。用于一致性检查。

	/*
	 * Get to the leaf entry and contained data entry address.
	 */
	lep = &leafhdr.ents[index];
	// lep指向找到的叶节点条目 这个条目包含了文件名对应的数据块地址，存储了该文件的目录项

	/*
	 * Point to the data entry.
	 */
	 /*通过lep中的地址信息计算出目录项在数据块中的实际位置，并将其存储到dep中。dep包含文件名
	   和对应的inode编号
	   其中lep->address是目录项所在数据块的偏移地址
	   xfs_dir2_dataptr_to_off用于将数据块指针转环卫内存中的偏移量
	 */
