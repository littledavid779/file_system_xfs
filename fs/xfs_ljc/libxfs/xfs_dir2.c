	...
	struct xfs_name		*ci_name) /* out: actual name if CI match */
{
	...
	ASSERT(S_ISDIR(VFS_I(dp)->i_mode)); // 检查dp是否是一个目录inode
	XFS_STATS_INC(dp->i_mount, xs_dir_lookup); // 更新xfs统计信息
	 ...
	 */
	args = kmem_zalloc(sizeof(*args), KM_NOFS); //分配空间初始化xfs_da_args结构体
	// 用来传递查找操作的参数
	args->geo = dp->i_mount->m_dir_geo;
	args->name = name->name;
	args->namelen = name->len;
	...
	lock_mode = xfs_ilock_data_map_shared(dp); // 获取当前目录inode的数据锁
	if (dp->i_df.if_format == XFS_DINODE_FMT_LOCAL) { // 本地格式存储
		rval = xfs_dir2_sf_lookup(args);
		goto out_check_rval;
	}

	rval = xfs_dir2_isblock(args, &v); // 块格式存储
	if (rval)
		goto out_free;
	if (v) {
	...
	rval = xfs_dir2_isleaf(args, &v); //叶格式存储
	if (rval)
		goto out_free;
	if (v)
		rval = xfs_dir2_leaf_lookup(args);
	else
		rval = xfs_dir2_node_lookup(args); //更复杂的节点格式
