	...
	int			stripe_align)
{
	...
	error = xfs_filestream_select_ag(ap, args, &blen); // 选择分组进行块分配
	if (error)
		return error;
	ASSERT(args->pag);
	 ...
	 * immediately.
	 */
	if (ap->tp->t_flags & XFS_TRANS_LOWMODE) { // 文件系统空间不足
		args->minlen = ap->minlen;
		ASSERT(args->fsbno == NULLFSBLOCK);
		goto out_low_space;
	}
...
{
	struct xfs_mount	*mp = ap->ip->i_mount;
	struct xfs_alloc_arg	args = { // 初始块分配参数
		.tp		= ap->tp,
		.mp		= mp,
		.fsbno		= NULLFSBLOCK,
	...
	args.maxlen = min(ap->length, mp->m_ag_max_usable); // 这里调整了要分配的块数，使其不会超过一个分配组（AG）的最大可用块数（mp->m_ag_max_usable）

	if ((ap->datatype & XFS_ALLOC_USERDATA) &&
	    xfs_inode_is_filestream(ap->ip))
		error = xfs_bmap_btalloc_filestreams(ap, &args, stripe_align); // 文件流模式
	else
		error = xfs_bmap_btalloc_best_length(ap, &args, stripe_align); // 最佳块分配
	if (error)
		return error;
 ...
 */
 // 索引过程主要由 xfs_iext_lookup_extent() 和 xfs_iext_next_extent() 这两个函数完成。
int
xfs_bmapi_read(
	struct xfs_inode	*ip,
	xfs_fileoff_t		bno,
	...
	uint32_t		flags)
{
	...
	// 读取文件的 extents 并加载到内存中
	error = xfs_iread_extents(NULL, ip, whichfork);
	if (error)
		return error;
	
	// 在文件中找到包含该块的extent 找到后将其存储在got 若找不到则表示是空洞或已达到末尾
	...
	while (bno < end && n < *nmap) {
		...
		if (got.br_startoff > bno) {
			/* Reading in a hole.  */
			mval->br_startoff = bno;
			mval->br_startblock = HOLESTARTBLOCK; // 标记空洞
			mval->br_blockcount =
		...
		xfs_bmapi_trim_map(mval, &got, &bno, len, obno, end, n, flags);// 裁剪
		xfs_bmapi_update_map(&mval, &bno, &len, obno, end, &n, flags);// 更新
		...
		// 在当前icur之后查找下一个extent，并更新icur的位置。
	...
	int			eof)
{
	 ...
	 */
	indlen = (xfs_extlen_t)xfs_bmap_worst_indlen(ip, alen); // 计算需要的索引块数
	ASSERT(indlen > 0);
	...
	xfs_mod_delalloc(ip->i_mount, alen + indlen);// 更新整个文件系统的延迟分配块的统计值
	...
	struct xfs_bmalloca	*bma)
{
	...
	if (whichfork == XFS_DATA_FORK || whichfork == XFS_COW_FORK) {
		...
		if (mp->m_dalign && bma->length >= mp->m_dalign) { // 检查对齐
			error = xfs_bmap_isaeof(bma, whichfork); // 检查末尾
			if (error)
				return error;
		}

		if (XFS_IS_REALTIME_INODE(bma->ip))
			return xfs_bmap_rtalloc(bma); // 实时分配
	}

	if (unlikely(XFS_TEST_ERROR(false, mp,
			XFS_ERRTAG_BMAP_ALLOC_MINLEN_EXTENT)))
		return xfs_bmap_exact_minlen_extent_alloc(bma); // 模拟错误
...
static int
xfs_bmapi_allocate(
	struct xfs_bmalloca	*bma)// bma包括文件 inode、请求分配的长度、偏移量、以及当前的扩展信息等
{
	struct xfs_mount	*mp = bma->ip->i_mount;
	int			whichfork = xfs_bmapi_whichfork(bma->flags);
	struct xfs_ifork	*ifp = xfs_ifork_ptr(bma->ip, whichfork);
	...
	if (bma->flags & XFS_BMAPI_CONTIG)
		...
		bma->minlen = 1;
// 物理块的分配
	if (bma->flags & XFS_BMAPI_METADATA) {// 元数据分配
		if (unlikely(XFS_TEST_ERROR(false, mp,
				XFS_ERRTAG_BMAP_ALLOC_MINLEN_EXTENT)))
			error = xfs_bmap_exact_minlen_extent_alloc(bma);
		else
			error = xfs_bmap_btalloc(bma); //元数据分配真正函数
	} else {
		error = xfs_bmap_alloc_userdata(bma);// 用户数据分配
	}
 ...
 * locking problems.
 */
 // 这个函数的核心任务是确保文件的指定区域有对应的物理存储空间，并进行相应的元数据更新。
int
xfs_bmapi_write(
	struct xfs_trans	*tp,		/* transaction pointer */
	struct xfs_inode	*ip,		/* incore inode */
